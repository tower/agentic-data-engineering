...
3. **App-type-specific deep checks:**
   ...
   **ASGI apps:**
   ...
   - Verify authentication middleware doesn't leak tokens in error responses
   - Check CORS configuration — overly permissive `allow_origins=["*"]` may be a risk
   - Verify request validation (input sanitization, no SQL injection in query handlers)
   - Check that error responses don't expose internal details (stack traces, config values)
