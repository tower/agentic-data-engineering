...
# App type detection
grep -l "RESTAPIConfig\|rest_api_resources\|rest_api_source" task.py 2>/dev/null && echo "SIGNAL:app_type=dlt"
...
| Signals | App Type |
|---------|----------|
| task.py contains `RESTAPIConfig` or `rest_api_resources` | `dlt` |
| task.py contains `dbtRunner` or `dbt_project.yml` exists | `dbt` |
| task.py exists but no dlt/dbt signals | `python` |
...
| App type | {dlt / dbt / python / empty} | {evidence} |
...
- **App type:** {dlt REST API pipeline / dbt project / Python script}
