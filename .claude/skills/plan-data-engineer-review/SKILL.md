...
2. Grep task.py (or main.py) for app type signals:
   - grep -l "RESTAPIConfig\|rest_api_resources\|rest_api_source" task.py → APP_TYPE: dlt
...
**SCORE 10 (python):** Script uses Tower parameters for date ranges (`PULL_DATE`, `END_DATE`), writes to Iceberg with upsert logic (PyIceberg `overwrite()` with `overwrite_filter`), and handles both backfill and incremental modes.
...
### PF-4: Write Disposition (dlt only)
...
### dbt-specific
8. "What if a source table schema changes between dbt runs? Will models fail gracefully or silently produce wrong results?"
