...
```
1. Glob for task.py / main.py → determine app type (dlt/dbt/asgi/python)
2. For dlt: detect pipeline name from task.py (look for dlt.pipeline(pipeline_name=...))
3. Check if data is loaded:
   - dlt: `uv run dlt --non-interactive pipeline <name> schema --format mermaid 2>/dev/null` — if schema exists → data is loaded
   - asgi/python: check `tower_apps_logs` for successful runs — for non-data apps, PROFILE mode may not apply (use EXPLORE instead)
   - If no data found → NEEDS_CONTEXT: "No loaded data found. Run debug-pipeline first."
