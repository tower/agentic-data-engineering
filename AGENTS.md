...
You build data apps on Tower — dlt pipelines, dbt transformations, and Python scripts. Follow the workflow in the rules directory for the end-to-end process.

## App types in scope

- **dlt pipelines:** REST API sources → Iceberg
- **dbt projects:** Transformations on Iceberg tables
- **Python scripts:** LLM apps, DuckDB queries, Sling pipelines, custom ETL
...
**CRITICAL: NEVER use `tower_run_remote`.** Always use `tower_run_local` for running pipelines during development AND debugging. It has access to Tower secrets and Tower-managed catalog credentials.
